...
extension ApiModel {
  ...
  func parse(_ type: ObjectType, _ statusMessage: String) async {
    ...
    switch type {
    case .amplifier:            amplifierStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kRemoved))
    case .atu:                  atu.parse( Array(statusMessage.keyValuesArray() ))
    case .bandSetting:          bandSettingStatus(Array(statusMessage.keyValuesArray().dropFirst(1) ), !statusMessage.contains(SharedModel.kRemoved))
    case .client:               preProcessClient(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kDisconnected))
    case .cwx:                  cwx.parse( Array(statusMessage.keyValuesArray().dropFirst(1) ))
    case .display:              preProcessDisplay(statusMessage)
    case .equalizer:            equalizerStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kRemoved))
    ...
    case .memory:               memoryStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kRemoved))
    case .meter:                meterStatus(statusMessage.keyValuesArray(delimiter: "#"), !statusMessage.contains(SharedModel.kRemoved))
    case .profile:              profileStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kNotInUse), statusMessage)
    case .radio:                ApiModel.shared.radio!.parse(statusMessage.keyValuesArray())
    case .slice:                sliceStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kNotInUse))
    case .tnf:                  tnfStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kRemoved))
    case .transmit:             preProcessTransmit(statusMessage)
    case .usbCable:             usbCableStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kRemoved))
    ...
    case .xvtr:                 xvtrStatus(statusMessage.keyValuesArray(), !statusMessage.contains(SharedModel.kNotInUse))
...
//  }
  ...
  private func preProcessClient(_ properties: KeyValuesArray, _ inUse: Bool = true) {
    ...
    if let handle = properties[0].key.handle {
      ...
      case SharedModel.kConnected:       parseConnection(properties: properties, handle: handle)
      case SharedModel.kDisconnected:    parseDisconnection(properties: properties, handle: handle)
      default:                      break
      }
    }
  }
  ...
  private func preProcessDisplay(_ statusMessage: String) {
    ...
    case ObjectType.panadapter.rawValue:  panadapterStatus(Array(statusMessage.keyValuesArray().dropFirst()), !statusMessage.contains(SharedModel.kRemoved) )
    case ObjectType.waterfall.rawValue:   waterfallStatus(Array(statusMessage.keyValuesArray().dropFirst()), !statusMessage.contains(SharedModel.kRemoved) )
    default: break
    }
  }
  ...
  private func preProcessInterlock(_ statusMessage: String) {
    ...
    switch properties[0].key {
    case ObjectType.bandSetting.rawValue:   bandSettingStatus(Array(statusMessage.keyValuesArray().dropFirst()), !statusMessage.contains(SharedModel.kRemoved) )
  ...
  private func preProcessTransmit(_ statusMessage: String) {
    ...
    switch properties[0].key {
    case ObjectType.bandSetting.rawValue:   bandSettingStatus(Array(statusMessage.keyValuesArray().dropFirst(1) ), !statusMessage.contains(SharedModel.kRemoved))
  ...
  private func parseConnection(properties: KeyValuesArray, handle: UInt32) {
    ...
    if handle == ApiModel.shared.connectionHandle { ApiModel.shared.clientInitialized = true }
    ...
    if let originalPacket = ApiModel.shared.activePacket, let packet = Listener.shared.packets[id: originalPacket.serial + originalPacket.publicIp] {
      ...
      if let myGuiClient = packet.guiClients[id: handle] {
        ...
        let guiClient = GuiClient(handle: handle,
                                  ...
                                  isThisClient: handle == ApiModel.shared.connectionHandle)
        Listener.shared.packets[id: originalPacket.serial + originalPacket.publicIp]!.guiClients[id: handle] = guiClient
  ...
  private func parseDisconnection(properties: KeyValuesArray, handle: UInt32) {
    ...
    // is it me?
    if handle == ApiModel.shared.connectionHandle {
  ...
  private func interlockStateChange(_ state: String) {
    let currentMox = ApiModel.shared.radio?.mox
    ...
    if state == Interlock.States.pttRequested.rawValue || state == Interlock.States.transmitting.rawValue {
      ...
      if currentMox == false { ApiModel.shared.radio?.mox = true }
      ...
      if currentMox == true { ApiModel.shared.radio?.mox = false  }
    }
  }
