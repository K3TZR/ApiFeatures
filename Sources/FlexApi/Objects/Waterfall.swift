...
//  }
  ...
  private let _apiModel = ApiModel.shared
//  private let _objectModel = ObjectModel.shared
  ...
  public func vitaProcessor(_ vita: Vita) {
    if _isStreaming == false {
      ...
      Task { await MainActor.run {  ApiModel.shared.waterfalls[id: vita.streamId]?.setIsStreaming() }}
    }
    ...
    vita.payloadData.withUnsafeBytes { ptr in
      ...
      if _expectedFrameNumber != _frames[_index].frameNumber {
        ...
        Task { await MainActor.run { ApiModel.shared.streamStatus[id: vita.classCode]?.errors += 1 }}
